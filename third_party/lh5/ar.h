...
/***********************************************************
        ...
        MODIFIED from upstream (ayfxedit-wx): error()'s fmt
        parameter is `const char *` instead of `char *` (a plain
        C++ requirement for passing string literals — not a
        behavior change), and a declaration for getc_arcfile()
        was added since it's now provided by a separate
        translation unit (VtxImport.cpp) instead of being
        textually #include'd ahead of these sources like the
        original ayfxedit did.
***********************************************************/
#include <stdio.h>
#include <limits.h>
